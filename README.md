...
1. Calls FeedLand's own JSON river API directly — `getriverfromcategory` (or `getriver` for "everything the user subscribes to") — server-side via `wp_remote_get()`, cached in a transient (`FEEDLAND_RIVERS_CACHE_TTL`, 10 minutes by default) so we're not hitting FeedLand on every page view.
