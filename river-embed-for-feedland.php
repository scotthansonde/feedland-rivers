 ...
 * Text Domain:       river-embed-for-feedland
...
if ( ! defined( 'FEEDLAND_RIVERS_PATH' ) ) {
	...
	function feedland_rivers_shortcode( $atts ): string {
		...
		if ( false === $srcdoc ) {
			return '<p class="feedlandRiversError">' . esc_html__( 'Unable to load the river right now.', 'river-embed-for-feedland' ) . '</p>';
		}

		$iframe_id = 'idFeedlandRivers' . wp_unique_id();

		feedland_rivers_enqueue_listener_script();
		...
		// preloading fresh content (see feedland_rivers_poll_listener_js()),
		...
		return '<div class="feedlandRiversWrap" style="position:relative;">'
			. '<iframe id="' . esc_attr( $iframe_id ) . '" class="feedlandRiversIframe" title="' . esc_attr__( 'FeedLand river', 'river-embed-for-feedland' ) . '" sandbox="allow-scripts allow-popups allow-popups-to-escape-sandbox" style="width:100%;height:600px;border:0;display:block;" srcdoc="' . $srcdoc_attr . '"' . $poll_attrs . '></iframe>'
			. '</div>';
	}

	/**
	 * Registers and enqueues the parent-page listener scripts (resize +
	 * poll/live-update) as a single inline script attached to a handle with no
	 * `src`, per WordPress's own pattern for inline-only scripts -- rather than
	 * printing raw `<script>` tags into the shortcode's own output. Only
	 * enqueued once per page even if the shortcode is used multiple times.
	 *
	 * @return void
	 */
	function feedland_rivers_enqueue_listener_script(): void {
		...
		wp_register_script( 'feedland-rivers-listener', false, array(), FEEDLAND_RIVERS_VERSION, true );
		wp_enqueue_script( 'feedland-rivers-listener' );
		wp_add_inline_script( 'feedland-rivers-listener', feedland_rivers_resize_listener_js() . feedland_rivers_poll_listener_js() );
	}

	/**
	 * The parent-page listener that resizes river iframes based on the
	 * postMessage sent by feedland_rivers_render_iframe_document().
	 *
	 * @return string Raw JavaScript, without `<script>` tags -- attached inline
	 *                via feedland_rivers_enqueue_listener_script().
	 */
	function feedland_rivers_resize_listener_js(): string {
		return 'window.addEventListener("message",function(e){if(!e.data||typeof e.data.feedlandRiversHeight!=="number")return;document.querySelectorAll(".feedlandRiversIframe").forEach(function(f){if(f.contentWindow===e.source){f.style.height=e.data.feedlandRiversHeight+"px";}});});';
	}

	/**
	 * The parent-page poll loop that refreshes a river iframe's content in
	 * place, without a full page reload, when new items are available. Returns
	 * '' when feedland_rivers_poll_interval() is zero, so polling is entirely
	 * absent from the page rather than merely idle.
	 *
	 ...
	 *                via feedland_rivers_enqueue_listener_script().
	 */
	function feedland_rivers_poll_listener_js(): string {
	...
	/**
	 * The WebSocket live-update trigger embedded in feedland_rivers_poll_listener_js()'s
	...
	function feedland_rivers_add_action_links( array $links ): array {
		...
		$settings_link = '<a href="' . esc_url( get_admin_url( null, 'options-general.php?page=' . $settings_slug ) ) . '">' . esc_html__( 'Settings', 'river-embed-for-feedland' ) . '</a>';
	...
	function feedland_rivers_admin_notices(): void {
		...
		$source = '' === $template_url
			? __( 'the built-in template', 'river-embed-for-feedland' )
			/* translators: %s: the configured template URL. */
			: sprintf( __( 'the template at %s', 'river-embed-for-feedland' ), '<code>' . esc_html( $template_url ) . '</code>' );

		printf(
			'<div class="notice notice-warning"><p><strong>%1$s</strong> %2$s</p></div>',
			esc_html__( 'River Embed:', 'river-embed-for-feedland' ),
			wp_kses(
				sprintf(
					/* translators: %1$s: description of which template, %2$s: the required id attribute. */
					__( 'No mount point was found in %1$s, so the river was appended to the end of the page instead of where the template expects it. Add an element with %2$s to the template.', 'river-embed-for-feedland' ),
					$source,
	...
	// Whichever copy loads first (e.g. an old "FeedLand Rivers" install left
	// active alongside this one under its new slug) defines
	// FEEDLAND_RIVERS_PATH and every feedland_rivers_*() function normally.
	// Every other copy hits this branch instead -- crucially, PHP hoists
	// unconditionally-declared top-level functions (like
	// feedland_rivers_shortcode() below) at compile time, before any line of
	// the file actually runs, so a plain early `return` here would not have
	// stopped WordPress from fataling on the redeclaration regardless -- the
	// whole plugin body above has to be nested inside this `if` (making
	// those declarations conditional, not hoisted) for that early exit to
	// mean anything.
	add_action(
		'admin_notices',
		function () {
			...
			printf(
				...
				esc_html__( 'Another FeedLand river plugin (an old "FeedLand Rivers" install, or a duplicate copy of this one) is already active. Only one can run at a time -- deactivate the other one.', 'river-embed-for-feedland' )
			);
		}
	);
}
